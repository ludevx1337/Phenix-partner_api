...
import { codeZoneParamSchema, msisdnParamSchema } from "@/lib/phenix/schemas";
...
export async function GET(request: Request) {
  try {
    ...
    const parsed = msisdnParamSchema.parse({ msisdn });
    ...
    const data = await phenixFetch({
      path: PhenixEndpoints.getDataRechargesByMsisdn,
      method: "GET",
      query: {
